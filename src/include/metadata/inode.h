...
class Inode {
  ...
  public:
    ...
    auto get_block_direct(usize idx) {
        if (idx >= this->nblocks - 1)
     ...
     */
    auto get_or_insert_indirect_block(
        std::shared_ptr<BlockAllocator> &allocator) -> ChfsResult<block_id_t> {
...
class InodeIterator {
     ...
     */
    explicit InodeIterator(Inode *inode, int idx)
        : cur_idx(idx), inode(inode) {}
